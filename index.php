<?php
  ...
  <ul class="vcard-details mb-3">', $pageArray[1]);
		...
		$pageArrayLocation = explode('<li itemprop="homeLocation" show_title="false" aria-label="Home location:', $profileLocation);

		if(sizeof($pageArrayLocation) > 1){

			$secondPageArrayLocation=explode('" class="vcard-detail pt-1 css-truncate css-truncate-target">', $pageArrayLocation[1]);
		...
		$pageArrayFollowers = explode('Followers
          <span class="Counter hide-lg hide-md hide-sm" >', $profileFollowers);

		if(sizeof($pageArrayFollowers) > 1){

			$secondPageArrayFollowers=explode('</span>', $pageArrayFollowers[1]);
		...
		$pageArrayFollowing = explode('Following
          <span class="Counter hide-lg hide-md hide-sm" >', $profileFollowing);

		if(sizeof($pageArrayFollowing) > 1){

			$secondPageArrayFollowing=explode('</span>', $pageArrayFollowing[1]);
		...
		$pageArrayRepo = explode('Repositories
          <span class="Counter hide-lg hide-md hide-sm">', $profileRepo);

		if(sizeof($pageArrayRepo) > 1){

			$secondPageArrayRepo=explode('</span>', $pageArrayRepo[1]);
	  	...
	  	<form id="search">
			...
			?></div>
		  	...
		  	<div class="row">
	  			...
	  			<div class="col" id="text">	
		  			<p><div class="bold inline">Repositories : &nbsp&nbsp</div><?php
